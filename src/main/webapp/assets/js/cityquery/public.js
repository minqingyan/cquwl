...
(function() {
	$(".proCitySelAll").live('click',function(event) {
		...
		$(".provinceCityAll").live('click',function(event) {
		...
		$(".provinceCityAll").find(".tabs").find("a").live('click',function() {
...
(function() {
	$(".proCitySelAll_Img").live('click',function(event) {
		event.stopPropagation();
...
function wrongMsg(object, msg) {
 ...
 (function() {
	...
	$div.find("a").live('click',function() {
		clearInterval(setFn);
		var j = $(this).index();
...
function sendCitiesAjax() {
	$.ajax({
		type: "get",
		url: 'queryAllCities.js',
		async: false,
		dataType: "json",
		success: function(data) {
            cities= data.content.cities;
			$("body").data("allExistCitys", cities);
			viewHotCities();
		},
		error: function(XMLHttpRequest, textStatus, errorThrown)
		 {
...
function sendAllProvinceAjax() {
	$.ajax({
		type: "post",
		url: 'queryAllProvinces.js',
		async: false,
		dataType: "json",
		success: function(data) {
			allProvinces = data.content.provinces;
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "post",
		url: "queryAllAreas.js",
		async: false,
		dataType: "json",
		success: function(data) {
			allAreas = data.content.areas;
...
function viewAllHotCities() {
	...
	function(i, city) {
		if (city.hotCity) {
			if(city.name == "清空") {
				$(".hotCityAll .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;color:red;' onclick=hotCityAddrInputAll(\'" + city.provinceId + "," + city.id + "," + city.name + "\') id='" + city.id + "' value='" + city.name + "'></a></li>");
			}else {
...
function allCountyPage(page) {
	...
	$("input.current2").removeClass("iGrays");
	if(cityCurrentName == "清空") {
		$("input.current2").val("");
		$(".provinceCityAll .tabs").find("#cityAll").removeClass("current");
		$(".provinceCityAll .tabs").find("#countyAll").removeClass("current");
